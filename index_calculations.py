"""
Hot Cracking Index Calculator for Scheil Results.

Processes individual alloy Scheil calculation CSV files to calculate hot cracking 
susceptibility indexes. Reads temperature and solidification data from timestamped 
result directories, applies mathematical models from indexes.py, and exports 
calculated index values to Excel format for analysis. Handles corrected initial 
temperatures from metadata files and sorts results by alloy index.
"""
import pandas as pd
import numpy as np
import json
...
def calculate_indexes():
    ...
    classic_dir = latest_results_dir / 'classic_scheil'
    solute_trap_dir = latest_results_dir / 'solute_trapping_scheil'
    ...
    for mode_name, data_dir in calc_modes.items():
        ...
        for csv_file in csv_files:
            ...
            df = df_load.copy()
            df = df[['Fraction_Solid', 'Temperature_C',
                     'Specific_Latent_Heat_J_per_g',
                     'Specific_Heat_Capacity_J_per_gK']].dropna()
        ...
        results_df['_sort_index'] = results_df['Sheet_Name'].apply(
            extract_index)
        results_df = results_df.sort_values('_sort_index').drop(
            '_sort_index', axis=1).reset_index(drop=True)

        all_results[mode_name] = results_df

    output_filename = 'alloy_index_results.xlsx'

    output_filename = latest_results_dir / 'alloy_index_results.xlsx'
